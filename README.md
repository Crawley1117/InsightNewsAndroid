...
### 最新版本：0.2.1(接口接入&功能更新）

### 用户及个人中心
- 实现了用户信息编辑，支持用户名、邮箱修改和头像上传。
- 实现了用户收藏话题列表的展示页面。

### 话题浏览与详情
- 实现了基于 `TabLayout` 和 `ViewPager2` 的分类话题浏览功能。
- 实现了话题详情页，分离资讯和评论区。
- 对接了话题收藏/取消收藏接口 (`POST /topic/favorite/{topicId}`), 可在详情页点击心形按钮操作。

### 搜索功能
- 实现了搜索页面，包含“历史记录”和“热搜词”模块。
- 对接了获取及一键清空搜索历史接口。
- 实现了过长历史记录的折叠与展开功能。
- 实现了搜索结果页，并支持在结果页就地刷新，发起新的搜索。
- 统一了搜索页与搜索结果页的UI风格（背景、顶部栏）。

### 评论系统
- 实现了嵌套评论（楼中楼）的UI展示。
- 对接了懒加载子评论接口 (`GET /topic/comment/replies/{commentId}`), 实现点击“查看回复”动态加载。
- 对接了评论点赞/取消点赞接口 (`POST /topic/comment/toggle-like/{commentId}`), 点击爱心图标可与后端同步状态。
- 对接了删除评论接口 (`DELETE /topic/delete/{commentId}`), 用户只能看到并删除自己的评论。
