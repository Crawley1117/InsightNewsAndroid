...
import com.example.insightnewsandroid.databinding.ActivityGuardianBinding;
...
public class GuardianActivity extends AppCompatActivity {
    ...
    private void showFeedback(String message, boolean isTerminal) {
        ...
        if (isTerminal) { // For final states like "加载失败" or "挑战完成"
            handler.postDelayed(this::finish, 2000);
        } else { // For intermediate feedbacks
            handler.postDelayed(() -> {
                binding.tvFeedback.setVisibility(View.GONE);
                moveToNextQuestion();
            }, 5000);
